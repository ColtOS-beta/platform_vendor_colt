...
if not depsonly:
    print("Device %s not found. Attempting to retrieve device repository from ColtOS Github (https://github.com/Colt-Enigma)." % device)
...
def is_in_manifest(projectpath):
    ...
    # ... and don't forget the colt snippet
    try:
        lm = ElementTree.parse(".repo/manifests/snippets/colt.xml")
        lm = lm.getroot()
    except:
        lm = ElementTree.Element("manifest")
...
def add_to_manifest(repositories):
    ...
    for repository in repositories:
        ...
        if is_in_manifest(repo_target):
            print('ColtOS/%s already fetched to %s' % (repo_name, repo_target))
            continue

        print('Adding dependency: ColtOS/%s -> %s' % (repo_name, repo_target))
        project = ElementTree.Element("project", attrib = {
            "path": repo_target,
            "remote": "github",
            "name": "ColtOS/%s" % repo_name,
...
def get_default_or_fallback_revision(repo_name):
    ...
    githubreq = urllib.request.Request("https://api.github.com/repos/ColtOS/" + repo_name + "/branches")
    add_auth(githubreq)
...
print("Repository for %s not found in the ColtOS Github repository list. If this is in error, you may need to manually add it to your local_manifests/roomservice.xml." % device)
